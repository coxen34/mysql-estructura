...
SELECT codigo_fabricante FROM fabricante f JOIN producto p ON f.codigo = p.codigo_fabricante;
SELECT DISTINCT codigo_fabricante FROM fabricante f JOIN producto p ON f.codigo = p.codigo_fabricante;
...
SELECT p.nombre,MIN(p.precio),f.nombre AS fabricante FROM producto p INNER JOIN fabricante f ON p.codigo_fabricante=f.codigo;
SELECT p.nombre,MAX(p.precio),f.nombre AS fabricante FROM producto p INNER JOIN fabricante f ON p.codigo_fabricante=f.codigo;
...
SELECT * FROM producto p INNER JOIN fabricante f ON p.codigo_fabricante=f.codigo IN (1,3,5);
...
SELECT d.nombre AS nombre_departamento , p.apellido1, p.apellido2, p.nombre FROM persona p LEFT JOIN departamento d ON p.id= d.id RIGHT JOIN profesor pr ON p.id=pr.id_profesor ORDER BY d.nombre, p.apellido1, p.apellido2, p.nombre;
SELECT p.nombre FROM persona p LEFT JOIN departamento d ON p.id= d.id LEFT JOIN profesor pr ON p.id=pr.id_departamento WHERE d.nombre IS NULL AND p.tipo='profesor';
SELECT d.nombre FROM departamento d LEFT JOIN profesor pr  ON d.id= pr.id_departamento WHERE pr.id_profesor IS NULL ;
SELECT p.nombre FROM profesor pr LEFT JOIN asignatura a ON pr.id_profesor=a.id_profesor LEFT JOIN persona p ON pr.id_profesor=p.id WHERE a.id_profesor IS NULL;
...
SELECT d.nombre AS nom_departament,COUNT(pr.id_profesor) AS nombre_professors FROM departamento d INNER JOIN profesor pr ON d.id=pr.id_departamento INNER JOIN persona p ON d.id=p.id GROUP BY d.id ORDER BY p.nombre DESC;
